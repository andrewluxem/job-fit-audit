...
5. **STOP and confirm before saving. This step is mandatory, even when the last answer seemed to complete everything.** Your reply after the final intake answer must contain no file creation. It shows a short summary (positioning line, level, titles, location and relocation, comp approach, dealbreakers, top proof points with their attribution, referral network, writing rules) and ends with: "Anything wrong or missing? Say **looks good** and I'll write your files." Only after the user replies do you fix anything flagged and write the files using `references/profile-template.md` and `scripts/resume.example.json` as the shapes.
